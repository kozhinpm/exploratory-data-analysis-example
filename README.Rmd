...
```
```{r, tidy: (TRUE), import csv}
test_price = read.csv("data.csv")
head(test_price)
```

0. Описательная статистика
```{r Находим самый частотный бренд, категорию, подкатегорию}
...
```{r статистика для самого частого бренда/ категории/ подкатегории отдельно}
...
```{r message=FALSE, warning=FALSE, boxplot}
...
```{r message=FALSE, warning=FALSE, clustering}
